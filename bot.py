...
button_finanses = KeyboardButton(text="Личные финансы")

keyboard = ReplyKeyboardMarkup(
    keyboard=[[button_register, button_exchange_rate], [button_tips, button_finanses]],
...
cursor.execute(
    "CREATE TABLE IF NOT EXISTS users (id INTEGER PRIMARY KEY, telegram_id INTEGER UNIQUE, name TEXT, category1 TEXT, category2 TEXT, category3 TEXT, expenses1 REAL, expenses2 REAL, expenses3 REAL)"
)
conn.commit()
...
async def send_start(message: Message):
    await message.answer(
        "Привет, я ваш личный финансовый помощник. Выберите одну из опций меню:",
...
async def exchange_rate(message: Message):
    url = "https://v6.exchangerate-api.com/v6/6b82be9fb172fda77e3d40ca/latest/USD"
    try:
...
async def category1(message: Message, state: FSMContext):
    ...
    await message.reply("Введите расходы по первой категории:")
...
async def category2(message: Message, state: FSMContext):
    ...
    await message.reply("Введите расходы по второй категории:")
...
async def category3(message: Message, state: FSMContext):
    ...
    await message.reply("Введите расходы по третьей категории:")
